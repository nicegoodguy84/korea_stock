...
def get_dynamic_theme_stocks():
    """실시간 거래대금 및 상승률 기준 상위 주도 테마의 종목들을 동적으로 수집합니다."""
    print(
        "🔥 [동적 엔진] 현재 시장의 실시간 주도 테마 및 구성 종목을 추적합니다..."
    )
        ...
        for theme_no in target_themes:
            ...
            try:
                with urllib.request.urlopen(
                    detail_req, timeout=5
                ) as detail_res:
                    detail_html = detail_res.read().decode(
                        "cp949", errors="ignore"
                    )
                stock_codes = re.findall(
                    r'href="/item/main\.naver\?code=(\d{6})"', detail_html
                )
                ...
                continue
        print(
            f"✅ 주도 테마 수집 완료: 실시간 급등 테마주 {len(theme_stock_codes)}개 확보"
        )
    except Exception as e:
        print(
            f"⚠️ 실시간 테마주 수집 중 오류 발생 (기본 시총 기반으로 우회 진행): {e}"
        )
...
def generate_chart_image(
    ...
    """현재 Base는 U자형 곡선, 직전 모든 Base는 타원형으로 차트 시각화"""
...
def generate_combined_html_report(
    ...
    """최종 통합 웹 보고서 템플릿 마크업 빌더 (면책 조항 포함)"""
...
<head>
    <meta charset="UTF-8">
    <title>미너비니 정통 VCP & 와인스타인 스테이지2 융합 초수익 리포트 (국장)</title>
...
"""
    ...
    # GitHub Pages 웹사이트 갱신을 위한 index.html 복사 저장
...
def get_combined_screener():
    print(
        "🚀 [와인스타인 2단계 X 미너비니 VCP 정통 스크리너] 시스템 가동"
    )
    ...
    try:
        ...
        marcap_col = next(
            (c for c in ["MarCap", "MarketCap", "시가총액"] if c in df_krx.columns),
            None,
        )
        df_large_cap = (
            df_krx.sort_values(by=marcap_col, ascending=False).head(400)
            if marcap_col
            else df_krx.head(400)
        )

        theme_codes = get_dynamic_theme_stocks()
        df_theme_stocks = df_krx[df_krx["Code"].isin(theme_codes)]
        df_targets = pd.concat(
            [df_large_cap, df_theme_stocks], ignore_index=True
    ...
    for row in valid_targets:
        ...
        try:
            ...
            w_stage2 = (current_close > ma150) and (
                ma150 > df_hist["MA150"].iloc[-20]
            )
            m_template = (
                (current_close > ma50)
                and (ma50 > ma150)
                and (ma150 > ma200)
                and (ma200 > df_hist["MA200"].iloc[-20])
            )
            ...
            if w_stage2 and m_template and trend_safety:
                ...
                tqdm.write(
                    f" 🎯 [타점포착] {code} ({name}) | RS: {rs_rating}점 | VCP비율: {vcp_ratio} | 스코어: {score}점"
                )
        except Exception:
            continue
    ...
    df_result.to_csv(
        f"정통_융합_추세돌파_스크리닝_{today_str}.csv",
        index=True,
        encoding="utf-8-sig",
    )
